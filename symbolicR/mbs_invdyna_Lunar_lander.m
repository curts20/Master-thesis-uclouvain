...
% Forward Kinematics 
  ...
  ALPHA27 = ALPHA26+BETA46*s.dpt(1,1);
  ALPHA37 = C7*(ALPHA35+BETA76*s.dpt(1,1))+S7*(ALPHA16+BS16*s.dpt(1,1));
  ...
  ALPHA29 = ALPHA26+BS56*s.dpt(2,2);
  ALPHA39 = C9*(ALPHA35+BETA86*s.dpt(2,2))+S9*(ALPHA16+BETA26*s.dpt(2,2));
  ...
  ALPHA211 = ALPHA26+BETA46*s.dpt(1,3);
  ALPHA311 = C11*(ALPHA35+BETA76*s.dpt(1,3))+S11*(ALPHA16+BS16*s.dpt(1,3));
...
% Backward Dynamics 
  ...
  Fs114 = -(s.frc(1,14)-s.m(14)*(s.l(3,14)*(OMp214+OM114*OM314)+C13*(ALPHA16+BETA26*s.dpt(2,4))-S13*(ALPHA35+BETA86*s.dpt(2,4))));
  ...
  Fs112 = -(s.frc(1,12)-s.m(12)*(s.l(3,12)*(OMp212+OM112*OM312)+C11*(ALPHA16+BS16*s.dpt(1,3))-S11*(ALPHA35+BETA76*s.dpt(1,3))));
  ...
  Fs110 = -(s.frc(1,10)-s.m(10)*(s.l(3,10)*(OMp210+OM110*OM310)+C9*(ALPHA16+BETA26*s.dpt(2,2))-S9*(ALPHA35+BETA86*s.dpt(2,2))));
  ...
  Fs18 = -(s.frc(1,8)-s.m(8)*(s.l(3,8)*(OMp28+OM18*OM38)+C7*(ALPHA16+BS16*s.dpt(1,1))-S7*(ALPHA35+BETA76*s.dpt(1,1))));
 ...
 Fs26*s.l(3,6)-OM26*(s.In(3,6)*OM16+s.In(6,6)*OM26+s.In(9,6)*OM36)+OM36*(s.In(2,6)*OM16+s.In(5,6)*OM26+s.In(6,6)*OM36)-s.dpt(2,2)*(Fq39*C9-Fs110*S9)-...
 s.dpt(2,4)*(Fq313*C13-Fs114*S13));
 ...
 s.In(9,6)*OM36)-OM36*(s.In(1,6)*OM16+s.In(2,6)*OM26+s.In(3,6)*OM36)+s.dpt(1,1)*(Fq37*C7-Fs18*S7)+s.dpt(1,3)*(Fq311*C11-Fs112*S11));
 ...
 Fq211*s.dpt(1,3)-Fq27*s.dpt(1,1)-OM16*(s.In(2,6)*OM16+s.In(5,6)*OM26+s.In(6,6)*OM36)+OM26*(s.In(1,6)*OM16+s.In(2,6)*OM26+s.In(3,6)*OM36)+s.dpt(2,2)*(...
 Fq39*S9+Fs110*C9)+s.dpt(2,4)*(Fq313*S13+Fs114*C13));
